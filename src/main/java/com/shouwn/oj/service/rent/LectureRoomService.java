...
public class LectureRoomService {
	...
	public HtmlPage selectBuilding(HtmlPage rentPage, int buildingNumber) {
		try {
			rentPage = clickBuilding(rentPage, BuildingType.valueOf(buildingNumber));
		/*	List<LectureRoom> lectureRooms = classRoomList(rentPage);*/
			return rentPage;
		} catch (IOException e) {
	...
	private HtmlPage clickBuilding(HtmlPage rentPage, BuildingType type) throws IOException {
	...
	public List<LectureRoom> classRoomList(HtmlPage rentPage) {
		...
		for (int i = 1; i < lectureRoomsTable.getRowCount(); i++) {
			...
			if (StringUtils.equals(classroomType.getClassroomName(), classroomName)) {
				lectureRooms.add(new LectureRoom( classroomType,people));
			}
		}
		return lectureRooms;
	}
