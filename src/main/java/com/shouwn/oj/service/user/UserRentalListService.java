...
public class UserRentalListService {
	...
	public HtmlPage rentalCancel(HtmlPage rentalPage, int idx) {
		int pageCount = 1;
		HtmlTable rentalListTable2 = (HtmlTable) rentalPage.getElementById("gv대여내역");

		try {
			for (int i = 1; i < rentalListTable2.getRowCount(); i++) {
				if (rentalListTable2.getRow(i).getChildElementCount() == 1) {
					for (HtmlAnchor anchor : rentalPage.getAnchors()) {
						if (anchor.asText().equals(Integer.toString(pageCount + 1))) {
							rentalPage = anchor.click();
							Thread.sleep(3000);
							++pageCount;
							i = 1;
							rentalListTable2 = (HtmlTable) rentalPage.getElementById("gv대여내역");
							}
							break;
						}
					}
